 ...
 *   const results = await html2pptxBatch(['slide1.html', 'slide2.html', ...], pptx, {
 *     minBottomMargin: 0.3,  // Lenient mode (default: 0.5)
 *     strictMode: false,      // Warnings instead of errors for minor violations
 *     concurrency: 3          // Process 3 slides in parallel
 *   });
 *
 * FEATURES:
 ...
 *   - Browser reuse for batch processing
 *   - Auto-fix suggestions for spacing issues
 *
 * VALIDATION:
 *   - Uses body width/height from HTML for viewport sizing
 *   - Throws error if HTML dimensions don't match presentation layout
 *   - Throws error if content overflows body (with overflow details)
 *   - Configurable bottom margin requirement (default: 0.5")
 *   - Warning mode for minor violations (non-blocking)
 *
 * RETURNS:
 *   { slide, placeholders, warnings? } where placeholders is an array of { id, x, y, w, h }
 */
...
// Helper: Generate spacing fix suggestions (moved before validateTextBoxPosition)
function generateSpacingFix(element, distanceFromBottom, minMargin) {
  const gap = minMargin - distanceFromBottom;
  
  if (gap > 0 && gap < 0.3) { // Small gap, easy to fix
    const gapPt = gap * 72; // Convert inches to points
    
    const suggestions = [];
    const currentPadding = parseFloat(element.style?.paddingBottom || element.style?.marginBottom || 0);
    const currentFontSize = parseFloat(element.style?.fontSize || 16);
    const currentLineHeight = parseFloat(element.style?.lineHeight || 1.5);
    
    // Suggestion 1: Reduce bottom padding/margin
    if (currentPadding > 0) {
      suggestions.push({
        type: 'padding',
        property: 'padding-bottom',
        current: `${currentPadding}pt`,
        suggested: `${Math.max(0, currentPadding - gapPt)}pt`,
        adjustment: `-${gapPt.toFixed(0)}pt`,
        css: `padding-bottom: ${Math.max(0, currentPadding - gapPt)}pt;`
      });
    }
    
    // Suggestion 2: Reduce font size slightly
    if (currentFontSize > 12) {
      suggestions.push({
        type: 'font',
        property: 'font-size',
        current: `${currentFontSize}pt`,
        suggested: `${currentFontSize - 1}pt`,
        adjustment: `-1pt`,
        css: `font-size: ${currentFontSize - 1}pt;`
      });
    }
    
    // Suggestion 3: Reduce line height
    if (currentLineHeight > 1.2) {
      suggestions.push({
        type: 'line-height',
        property: 'line-height',
        current: currentLineHeight.toString(),
        suggested: `${Math.max(1.2, currentLineHeight - 0.1)}`,
        adjustment: `-0.1`,
        css: `line-height: ${Math.max(1.2, currentLineHeight - 0.1)};`
      });
    }
    
    return {
      autoFixable: suggestions.length > 0,
      gap: `${gap.toFixed(2)}" (${gapPt.toFixed(0)}pt)`,
      suggestions,
      recommended: suggestions[0] || null
    };
  }
  
  return { autoFixable: false, gap: `${gap.toFixed(2)}"` };
}

function validateTextBoxPosition(slideData, bodyDimensions, options = {}) {
  const errors = [];
  const warnings = [];
  const fixes = [];
  
  // IMPROVEMENT: Configurable margin threshold
  const minBottomMargin = options.minBottomMargin || 0.5; // Default 0.5", but configurable
  const strictMode = options.strictMode !== false; // Default strict, but can be lenient
  const warningThreshold = minBottomMargin * 0.6; // Warn at 60% of required margin
  const autoFix = options.autoFix || false; // Generate fix suggestions
  ...
  for (const el of slideData.elements) {
    ...
    if (['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'list'].includes(el.type)) {
      ...
      if (fontSize > 12 && distanceFromBottom < minBottomMargin) {
        ...
        // IMPROVEMENT: Warning mode for minor violations
        if (!strictMode && distanceFromBottom >= warningThreshold) {
          const warningMsg = `Text box "${textPrefix}" is close to bottom edge ` +
            `(${distanceFromBottom.toFixed(2)}" from bottom, recommended ${minBottomMargin}")`;
          warnings.push(warningMsg);
          
          // Generate fix suggestions if auto-fix enabled
          if (autoFix) {
            const fix = generateSpacingFix(el, distanceFromBottom, minBottomMargin);
            if (fix.autoFixable) {
              fixes.push({
                element: textPrefix,
                warning: warningMsg,
          ...
          let errorMsg = `Text box "${textPrefix}" ends too close to bottom edge ` +
            `(${distanceFromBottom.toFixed(2)}" from bottom, minimum ${minBottomMargin}" required)`;
          
          // IMPROVEMENT: Add auto-fix suggestions to error messages
          if (autoFix) {
            const fix = generateSpacingFix(el, distanceFromBottom, minBottomMargin);
            if (fix.autoFixable && fix.recommended) {
              errorMsg += `\n   💡 Auto-fix: ${fix.recommended.css}`;
              if (fix.suggestions.length > 1) {
                errorMsg += `\n   Or reduce font size: ${fix.suggestions.find(s => s.type === 'font')?.css || 'N/A'}`;
              }
              errorMsg += `\n   Gap to fix: ${fix.gap}`;
            }
          }
...
// IMPROVEMENT: Core processing function (can be called with existing browser/page)
async function html2pptxSingle(htmlFile, pres, options = {}) {
  const {
    tmpDir = process.env.TMPDIR || '/tmp',
    slide = null,
    page = null,  // NEW: Allow passing existing page
    browser = null  // NEW: Allow passing existing browser
  } = options;
  ...
  try {
    // IMPROVEMENT: Reuse browser if provided, otherwise create new
    if (!browserInstance) {
      const launchOptions = { 
        env: { TMPDIR: tmpDir },
        headless: true,
    ...
    await pageInstance.goto(`file://${filePath}`);

    const bodyDimensions = await getBodyDimensions(pageInstance);
    ...
    const slideData = await extractSlideData(pageInstance);
    ...
    // IMPROVEMENT: Use improved validation with options
    const textBoxValidation = validateTextBoxPosition(slideData, bodyDimensions, {
      minBottomMargin: options.minBottomMargin,
      strictMode: options.strictMode,
      autoFix: options.autoFix
    });
    
    if (textBoxValidation.errors.length > 0) {
      validationErrors.push(...textBoxValidation.errors);
    }
    if (textBoxValidation.warnings.length > 0) {
      validationWarnings.push(...textBoxValidation.warnings);
    }
    if (textBoxValidation.fixes.length > 0) {
      validationFixes.push(...textBoxValidation.fixes);
    }
    ...
    // IMPROVEMENT: Only throw errors, warnings are non-blocking
    ...
    // IMPROVEMENT: Include warnings and fixes in result (non-blocking)
    if (validationWarnings.length > 0) {
      result.warnings = validationWarnings;
    }
    if (validationFixes.length > 0) {
      result.fixes = validationFixes;
    }
    
    return result;
  } finally {
    // Only close if we created it
    if (shouldClosePage && pageInstance) {
      await pageInstance.close();
    }
    // Don't close browser if it was passed in (for reuse)
...
async function html2pptx(htmlFile, pres, options = {}) {
  try {
    return await html2pptxSingle(htmlFile, pres, options);
  } catch (error) {
...
// IMPROVEMENT: Batch processing with browser reuse
async function html2pptxBatch(htmlFiles, pres, options = {}) {
  const {
    tmpDir = process.env.TMPDIR || '/tmp',
    concurrency = 1,  // Process slides sequentially by default, can be increased
    minBottomMargin = 0.5,
    strictMode = true,
    autoFix = false
  } = options;
  ...
  try {
    // Process slides with specified concurrency
    if (concurrency === 1) {
      // Sequential processing (simpler, more reliable)
      for (let i = 0; i < htmlFiles.length; i++) {
        const htmlFile = htmlFiles[i];
        try {
          const result = await html2pptxSingle(htmlFile, pres, {
            ...options,
            browser,
            minBottomMargin,
            strictMode,
          ...
          results.push({ slide: i + 1, file: htmlFile, result });
          
          if (result.warnings) {
            allWarnings.push(...result.warnings.map(w => `Slide ${i + 1}: ${w}`));
          }
          if (result.fixes) {
            allFixes.push(...result.fixes.map(f => ({ slide: i + 1, ...f })));
          }
        } catch (error) {
          allErrors.push(`Slide ${i + 1} (${htmlFile}): ${error.message}`);
      ...
      for (let i = 0; i < htmlFiles.length; i += concurrency) {
        chunks.push(htmlFiles.slice(i, i + concurrency).map((file, idx) => ({
          file,
          index: i + idx
        })));
      }
      ...
      for (const chunk of chunks) {
        const chunkResults = await Promise.all(
          chunk.map(async ({ file, index }) => {
            try {
              const result = await html2pptxSingle(file, pres, {
                ...options,
                browser,
                minBottomMargin,
                strictMode,
              ...
              return { slide: index + 1, file, result, error: null };
            } catch (error) {
              return { slide: index + 1, file, result: null, error: error.message };
            }
          })
        );
        ...
        for (const chunkResult of chunkResults) {
          if (chunkResult.error) {
            allErrors.push(`Slide ${chunkResult.slide} (${chunkResult.file}): ${chunkResult.error}`);
          } else {
            results.push(chunkResult);
            if (chunkResult.result.warnings) {
              allWarnings.push(...chunkResult.result.warnings.map(w => `Slide ${chunkResult.slide}: ${w}`));
            }
            if (chunkResult.result.fixes) {
              allFixes.push(...chunkResult.result.fixes.map(f => ({ slide: chunkResult.slide, ...f })));
            }
          }
        }
      }
  ...
  return {
    ...
    fixes: allFixes.length > 0 ? allFixes : null
  };
}

// Export both single and batch functions
module.exports = html2pptx;
module.exports.html2pptx = html2pptx;
module.exports.html2pptxSingle = html2pptxSingle;
module.exports.html2pptxBatch = html2pptxBatch;
module.exports.generateSpacingFix = generateSpacingFix;