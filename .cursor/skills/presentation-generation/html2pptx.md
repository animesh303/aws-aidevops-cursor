...
Process multiple slides efficiently with a single browser instance:
...
if (batchResult.fixes) {
  console.log('Auto-fix suggestions:', batchResult.fixes);
}
```
...
```javascript
// Single slide with lenient validation
const { slide, placeholders, warnings } = await html2pptx('slide.html', pptx, {
  minBottomMargin: 0.3,  // Allow 0.3" minimum (default: 0.5")
  strictMode: false,     // Warnings instead of errors (default: true)
  autoFix: true          // Generate fix suggestions (default: false)
});

// Warnings are non-blocking - presentation still generates
if (warnings) {
  console.warn('Minor spacing issues:', warnings);
}
```
...
- `options` (object, optional):
  ...
  - `minBottomMargin` (number): Minimum bottom margin in inches (default: `0.5`, can be `0.3` for lenient mode)
  - `strictMode` (boolean): If `false`, minor violations become warnings instead of errors (default: `true`)
  - `autoFix` (boolean): Generate fix suggestions for spacing issues (default: `false`)
  - `page` (object): Existing Playwright page to reuse (for batch processing)
  - `browser` (object): Existing Playwright browser to reuse (for batch processing)
...
```javascript
{
    ...
    warnings: [string]?,           // Non-blocking warnings (if strictMode: false)
    fixes: [object]?               // Auto-fix suggestions (if autoFix: true)
}
```
...
```javascript
await html2pptxBatch(htmlFiles, pres, options)
```

**Parameters:**
- `htmlFiles` (array): Array of HTML file paths
- `pres` (pptxgen): PptxGenJS presentation instance
- `options` (object, optional):
  - All options from `html2pptx()` plus:
  - `concurrency` (number): Number of slides to process in parallel (default: `1`)

**Returns:**
```javascript
{
    results: [                     // Array of successful slide results
        { slide: number, file: string, result: object },
        ...
    ],
    errors: [string]?,             // Array of error messages (if any)
    warnings: [string]?,           // Array of warning messages (if any)
    fixes: [object]?                // Array of fix suggestions (if any)
}
```
...
5. **Bottom margin spacing** - Configurable threshold (default: 0.5", can be set to 0.3" for lenient mode)

**All validation errors are collected and reported together** in a single error message, allowing you to fix all issues at once instead of one at a time.

#### Validation Modes

- **Strict Mode (default)**: All violations are errors, generation stops
- **Lenient Mode**: Minor violations (60-100% of threshold) become warnings, generation continues
- **Auto-Fix Mode**: Generates specific CSS fix suggestions for spacing issues

#### Validation Options

```javascript
{
  minBottomMargin: 0.5,    // Minimum bottom margin in inches (default: 0.5")
  strictMode: true,        // Strict validation (default: true)
  autoFix: false           // Generate fix suggestions (default: false)
}
```
...
### Batch Processing Example (NEW - Faster)

```javascript
const pptxgen = require('pptxgenjs');
const { html2pptxBatch } = require('./html2pptx');

async function createPresentationBatch() {
    const pptx = new pptxgen();
    pptx.layout = 'LAYOUT_16x9';
    pptx.author = 'Your Name';
    pptx.title = 'My Presentation';
    ...
    const htmlFiles = [
        'slides/slide-01-title.html',
        'slides/slide-02-agenda.html',
        'slides/slide-03-content.html',
        // ... all slides
    ];

    // Process all slides with optimizations
    const batchResult = await html2pptxBatch(htmlFiles, pptx, {
        minBottomMargin: 0.3,      // Lenient mode
        strictMode: false,          // Warnings instead of errors
        autoFix: true,              // Generate fix suggestions
        concurrency: 1              // Sequential (safe) or 3-5 (faster)
    });
    ...
    // Add charts to slides with placeholders
    batchResult.results.forEach(({ slide, result }) => {
        if (result.placeholders && result.placeholders.length > 0) {
            // Add chart to first placeholder
            result.slide.addChart(pptx.charts.BAR, chartData, result.placeholders[0]);
        }
    });
...
createPresentationBatch().catch(console.error);
```
